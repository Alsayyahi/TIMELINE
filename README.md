...
Note 2: The sentence boundaries (first two trigger words, last two trigger words) for each sentence in each document can be found in the (sentence_boundries.csv) file.

Note 3: Before the annotation process, we truncated 2-3 sentences at the end of four news articles (10,43,44 and 46) because when the articles were processed to generate the BERT embedding for each token, we received a message that said that BERT has a max length limit of tokens which is 512.
